...
abstract class MyList[+A] {
  ...
  // EXERCISES - PART 2: higher-order functions
...
case object Empty extends MyList[Nothing] { // CASE OBJECT!! → implkemented equals and haschode + Made my list serializable!
...
case class Cons[+A](h: A, t: MyList[A]) extends MyList[A] {
   ...
   */
  def ++[B >: A](list: MyList[B]): MyList[B] = new Cons(h, t ++ list) // Cons with head and tail + list
  /*
    [1,2].flatMap(n => [n, n+1])
...
object ListTest extends App {
  ...
  println(cloneListOfIntegers == listOfIntegers) // because of the keyword CASE!!!!
  ...
  // for comprehensions (mapflatmapfilterfor)
